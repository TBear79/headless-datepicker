...
export class HeadlessDatepicker {
    ...
    public getSelectedDates(): HdpDate[] { return this.selectedDates }

    public getRange(startDate: Date, endDate: Date): HdpDate[] {
        let dates = []
    ...
    public getMonths(months: HdpYearMonthPair[], mode: HdpCalendarMode, oneBasedMonth: boolean): HdpMonth[] {
    ...
    private createHdpDate(date: Date, isAdjacent: boolean): HdpDate { 
    ...
    private createAdjacentDateEntry(momentDate: moment.Moment, showAdjacentMonths: boolean): HdpDate { 
        return showAdjacentMonths ? this.createHdpDate(momentDate.toDate(), true) : null
    }
    
    private getWeekDays(range: HdpDate[]): WeekDayInfo { 
        const weekDays = {
            full: [],
            short: [],
            min: []
    ...
    private getMonthNames(range: HdpDate[], monthOffset: number): HdpMonthInfo {
    ...
    private getAdjacentBefore(momentDate: moment.Moment, showAdjacentMonths: boolean): HdpDate[] { 
    ...
    private getAdjacentAfter(momentDate: moment.Moment, showAdjacentMonths: boolean): HdpDate[] { 
        const dayOfWeek = momentDate.weekday()
        const clonedDate = momentDate.clone()
    ...
    private splitIntoWeeks(range: HdpDate[]): HdpDate[][] { 
        const weeks = []
    ...
    private exactMode(range: HdpDate[]): HdpDate[][] { 
        return this.splitIntoWeeks(range)
    }

    private fillMode(range: HdpDate[], showAdjacentMonths: boolean): HdpDate[][] { 
    ...
    private adjacentMode(range: HdpDate[]): HdpDate[][] { 
        return this.fillMode(range, true)
    }
    
    private fixedMode(range: HdpDate[]): HdpDate[][] { 
    ...
    private getWeeks(range: HdpDate[], mode: HdpCalendarMode): HdpDate[][] { 
...
export interface HdpMonth {
    weekDayInfo: WeekDayInfo
    year: number
    monthInfo: HdpMonthInfo
    weeks: HdpDate[][]
}
